...
public class Estacionamiento {
    ...
    private int DPlazas;
    private int DPisos;
    private Auto [][] matriz=new Auto [DPlazas][DPisos];
    ...
    public Estacionamiento (int DPlazas, int DPisos) {
        this.DPlazas = DPlazas;
        this.DPisos = DPisos;
    }
        ...
        for (i=0; i<this.DPlazas; i++)
            for (p=0; p<this.DPisos; p++)
                if (this.matriz[i][p]!=null && this.matriz[i][p].getPatente()==patente){
                    aux="El auto con la patente "+patente+" se encuentra en el piso "+p+", plaza "+i+".";
                    i=this.DPlazas;
                    p=this.DPisos;
                }
        return aux;
